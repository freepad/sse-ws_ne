    ...
    ws.on('message', (m) => {
        ...
        ws.onclose = (e) => {
            ...
            if (e.code === 1001) {
                ...
                if ('id' in message)
                    db['logins'] = db['logins'].filter((item) => item['id'] !== message['id']);
                /** FILTER: Формируем новый список User-ов.
                     Кто покинул страницу - удаляется из списка */
                else {
                    db['logins'] = db['logins'].filter((item) => item['login'] !== message['newLogin']);
                }
        ...
        if (url.indexOf('/login') >= 0 && url.length > 1) {
            ...
            const result = db['logins'].find((elem) => elem['login'] === message['newLogin']);
            if (result === undefined) {
                ...
                newClient = {};
            wss.clients.forEach((client) => client.send(JSON.stringify(newClient)));
        }
             ...
             */
            const login = db['logins'].filter((item) => { if (item['id'] === onePost['id'])
                return item['login']; });
...
function makeUniqueId(str, database) {
    const respons = database.some((item) => { if (item.id === str)
        str; });
    if (respons)
...
function makePostId(ind, database) {
    ind++;
    const respons = database.some((item) => { if (item.idPost === (ind - 1))
        (ind - 1); });
    if (respons)
...
function poster(elem) {
    ...
    Array.from(wss.clients)
        .filter((clients) => clients.readyState === WS.OPEN)
        .forEach((clients) => {
